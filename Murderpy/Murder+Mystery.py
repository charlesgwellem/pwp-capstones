...
get_ipython().set_next_input('Pretty sinister stuff! Luckily, in addition to this bold-faced admission, we have the introduction letters of the three contestants. Maybe there is a way to use this information to determine who the author of this murder letter is');get_ipython().run_line_magic('pinfo', 'is')
...
SURE you think you know him! You've seen him smiling for the cameras,
laughing, joking, telling stories, waving his money around like a prop but off camera he was a sinister beast,
a cruel cruel taskmaster, he treated all of us like slaves, like cattle, like animals! Do you remember Lindsay, 
she was the first to go, he called her such horrible things that she cried all night, keeping up all up, crying, 
crying, and more crying, he broke her with his words. I miss my former cast members, all of them very much.
And we had to live with him, live in his home, live in his power, deal with his crazy demands. AND FOR WHAT! 
DID YOU KNOW THAT THE PRIZE ISN'T REAL? He never intended to marry one of us! The carrot on the stick was gone, 
all that was left was stick, he told us last night that we were all a terrible terrible disappointment and none of us 
would ever amount to anything, and that regardless of who won the contest he would never speak to any of us again! 
It's definitely the things like this you can feel in your gut how wrong he is! Well I showed him, he got what he 
deserved all right, I showed him, I showed him the person I am! I wasn't going to be pushed around any longer, 
and I wasn't going to let him go on pretending that he was some saint when all he was was a sick sick twisted man 
who deserved every bit of what he got. The fans need to know, Jay Stacksby is a vile amalgamation of all things evil 
and bad and the world is a better place without him.
"""

lily_trebuchet_intro = """
Salutations. My name? Myrtle. Myrtle Beech. I am a woman of simple tastes. I enjoy reading, thinking, 
and doing my taxes. I entered this competition because I want a serious relationship. I want a commitment. 
...
Can you imagine? I like every action I take to have a measurable effect. When I see a movie, 
I like to walk away with insights that I did not have before. When I take a bike ride, 
there better be a worthy destination at the end of the bike path. Jay seems frivolous at times. 
This worries me. However, it is my staunch belief that one does not make and keep money without having a modicum of 
discipline. As such, I am hopeful. I will now list three things I cannot live without. Water. Emery boards. Dogs. 
...
household chores? Dishes, oh yes it's definitely the dishes, I just hate doing them, don't you?
Who is your favorite actor and why? Hmm, that's a hard one, but I think recently I'll have to go with Michael B. 
Jordan, every bit of that man is handsome, HANDSOME! Do you remember seeing him shirtless? I can't believe what he does 
for the cameras! Okay okay next question, what is your perfect date? Well it starts with a nice dinner at a delicious but 
small restaurant, you know like one of those places where the owner is in the back and comes out to talk to you and ask 
you how your meal was. My favorite form of art? Another hard one, but I think I'll have to go with music, music you can
feel in your whole body and it is electrifying and best of all, you can dance to it! Okay final question, let's see,
...
they support me in everything I do. I know Jay Stacksby is a handsome man and all of us want to be the first to walk down 
the aisle with him, but I think he might truly be the one for me.
Okay that's it for the bio, I hope you have fun watching the show! 
"""

gregg_t_fishy_intro = """
A good day to you all, I am Gregg T Fishy, of the Fishy Enterprise fortune. I am 37 years young.
An adventurous spirit and I've never lost my sense of childlike wonder. 
I do love to be in the backyard gardening and I have the most extraordinary time when I'm fishing. 
Fishing for what, you might ask? Why, fishing for compliments of course! I have a stunning pair of radiant blue eyes. 
They will pierce the soul of anyone who dare gaze upon my countenance.
...
beguiles my every day life. Every fiber of my being scintillates and vascillates with extreme pleasure 
during one of these charming anecdotes and significantly pleases my beautiful personage. 
...
def get_average_sentence_length(text):
    ...
    sentences_in_text = []
    temp_string = "" 
    list_of_chars_in_sentence = []
    word_counter = 0
    list_of_words_only_per_sentence = []
    temp_list = []
    sentence_lengths = []
    ...
    for char in text:
        temp_string += char
        if "." in temp_string:
            sentences_in_text.append(temp_string)
            temp_string = ""
    if temp_string:
        sentences_in_text.append(temp_string)
    for sentence in sentences_in_text:
        list_of_chars_in_sentence.append(sentence.split(" "))
    
    for sentence in list_of_chars_in_sentence:
        for char in sentence:
            if char == '':
                continue
            temp_list.append(char)
        
        list_of_words_only_per_sentence.append(temp_list)
        sentence_lengths.append(len(temp_list))
        temp_list = []
    if temp_list:
        sentence_lengths.append(len(temp_list))  
    
    """
    the nested for loop below can also be replaced with 
    sum(sentence_lengths)/len(sentence_lengths)
    """
    
    for sentence in list_of_words_only_per_sentence:
        for char in sentence:
            word_counter += 1
    average_of_all_sentence_lengths = word_counter/len(sentence_lengths)
   
    return average_of_all_sentence_lengths
...
class TextSample:
    ...
    def __repr__(self):
        return "author's name: {name}, sentence length: {length}".format(name = self.author, length = self.average_sentence_length)
...
def prepare_text(text):
    ...
    for letter in text:
        ...
        if ' ' in intermediate_string:
            list_without_punctuation.append(intermediate_string)
            intermediate_string = ""
    if intermediate_string:
        list_without_punctuation.append(intermediate_string)
        
    for word in list_without_punctuation:
        final_clean_list += word.strip(".").split()
...
def ngram_creator(text_list):
    ...
    while i < (len(text_list)-1):
        ngram_created.append(text_list[i] + ' ' + text_list[i + 1])
        i += 1
    return ngram_created
...
def frequency_comparison(table1, table2):
    ...
    table2_keys = table2.keys()
    table1_keys = table1.keys()
    ...
    for a_key in table2.keys():
        if a_key not in table1_keys:
            appearances += table2[a_key]
            
    frequency_comparison = mutual_appearances/appearances
